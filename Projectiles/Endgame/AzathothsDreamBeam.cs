...
namespace AncientSandsMod.Projectiles.Endgame
{
	public class AzathothsDreamBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / AzathothsDreamHoldout.NumBeams;
		...
		private const float MaxBeamLength = 2100f;
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<AzathothsDreamBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<AzathothsDreamHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / AzathothsDreamHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > AzathothsDreamHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - AzathothsDreamHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			if (chargeRatio < 1f)
			{
				...
				beamSpread = MathHelper.Lerp(MaxBeamSpread, 0.2f, chargeRatio);
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * AzathothsDreamHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / AzathothsDreamHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
