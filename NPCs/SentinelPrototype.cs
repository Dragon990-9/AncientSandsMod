...
namespace AncientSandsMod.NPCs
{
	...
	internal class SentinelPrototypeHead : ExampleWorm
	{
		...
		public override void CustomBehavior() {
			if (Main.netMode != NetmodeID.MultiplayerClient) {
				...
				if (attackCounter <= 0 && Vector2.Distance(npc.Center, target.Center) < 200 && Collision.CanHit(npc.Center, 1, 1, target.Center, 1, 1)) {
					...
					int projectile = Projectile.NewProjectile(npc.Center, direction * 20, ModContent.ProjectileType<Projectiles.EnemyGreenLaser>(), 30, 0, Main.myPlayer);
	...
	internal class SentinelPrototypeBody : ExampleWorm
	{
	...
	internal class SentinelPrototypeTail : ExampleWorm
	{
	...
	public abstract class ExampleWorm : SentinelPrototype
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Sentinel Prototype");
		}

		public override void Init() {
			minLength = 20; //MIN AND MAX SEGMENTS
			maxLength = 20;
			tailType = ModContent.NPCType<SentinelPrototypeTail>();
			bodyType = ModContent.NPCType<SentinelPrototypeBody>();
			headType = ModContent.NPCType<SentinelPrototypeHead>();
			speed = 20f;
			turnSpeed = 0.1f;
		}
	}
	...
	public abstract class SentinelPrototype : ModNPC
	{
		/* ai[0] = follower
		 * ai[1] = following
		...
		public override void AI() {
			...
			if (Main.player[npc.target].dead && npc.timeLeft > 20) {
				npc.timeLeft = 20;
			}
			if (Main.netMode != NetmodeID.MultiplayerClient) {
				if (!tail && npc.ai[0] == 0f) {
			...
			Vector2 npcPos = npc.Center;
				   ...
				   Lighting.AddLight(npcPos, 0, 1, 0);
