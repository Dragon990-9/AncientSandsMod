    ...
    public class ChronosGlobalNPC : GlobalNPC
    {
        public override void NPCLoot(NPC npc)
        {
           ...
           //DEFENSE DRONE
            ...
            //KING CAVE BAT BOSS
            if (npc.type == ModContent.NPCType<NPCs.Bosses.KingBat>())
            {
                  if (Main.rand.Next(3) == 1)   //item rarity add {} if ur gonna use it
                {

                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("BatBow"), 1);

                }
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("BatFang"), Main.rand.Next(30, 70));
